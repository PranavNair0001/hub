...
while [ $i -lt ${#toFetch[@]} ]; do
    json="$json{\"path\":\"${toFetch[$i]}\",\"target_path\":\"target/`echo "${toFetch[$i]}" | cut -d "/" -f 4`\",\"repo\":{\"url\":\"${urls[$i]}\",\"owner\":\"`echo ${urls[$i]} | cut -d "/" -f 4`\",\"repo\":\"`echo ${urls[$i]} | cut -d "/" -f 5`\",\"tag\":\"v`echo ${toFetch[$i]} | cut -d "/" -f 3`\",\"checkout\":\"`echo ${urls[$i]} | cut -d "/" -f 4-5`\"}},";
    i=`expr $i + 1`;
done
